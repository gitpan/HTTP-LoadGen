...
This is only used when a SSL connection is made. Default is
C<< {cache=>1} >> meaning it is allowed to use an SSL session cache.
If you need to verify the identity of the server it can be configured here.
...
Example:
 ...
 {
  '7273' => [7, 0],     # PID 7273 exits normally with code 7
  '7275' => [0, 11],    # PID 7275 has been killed by signal 11
  '7274' => [8, 0],     # PID 7274 exits normally with code 8
 }
...
=head1 EXAMPLE CONFIGURATION
 ...
 use strict;
 use HTTP::LoadGen::Run;	# use RQ_, RC_ and KEEPALIVE_ constants
 use HTTP::LoadGen::Logger;	# a logger
 use HTTP::LoadGen::ScoreBoard;	# and a scoreboard
 use Math::Random::MT;		# to implement a thread-specific RNG
 ...
 my $logger;
 {
  ...
  ParentInit=>sub {
    ...
    # create scoreboard
    HTTP::LoadGen::ScoreBoard::init 'scoreboard.sb',
	@{HTTP::LoadGen::options()}{qw/NWorker SbSlotsz SbExtra/};
  },
  ParentExit=>sub {
    # no parameters
